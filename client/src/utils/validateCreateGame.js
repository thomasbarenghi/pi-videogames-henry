    ...
    switch (fieldName) {
      case 'name':
        if (!form.name) {
          ...
          errors.name = 'El nombre de usuario no debe tener más de 40 caracteres'
        }
        break
      case 'description':
        if (!form.description) {
          ...
          errors.description = 'La descripción no debe tener más de 1000 caracteres'
        }
        break
      case 'released':
        ...
        if (!form.released) {
          ...
          errors.released = 'La fecha de lanzamiento no puede ser mayor a la fecha actual'
        } else if (!regexFecha.test(form.released)) {
          errors.released = 'La fecha de lanzamiento debe tener el formato YYYY-MM-DD'
        }
        break
      case 'rating':
        if (!form.rating) {
      ...
      case 'background_image':
        ...
        if (!form.background_image) {
          ...
          errors.background_image = 'El link de la imagen debe ser una url válida tipo https'
        }
        break
      case 'token':
        if (!form.token) {
