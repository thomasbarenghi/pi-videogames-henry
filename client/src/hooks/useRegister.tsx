...
const useRegister = () => {
  ...
  const handleNamesErrors = (e: React.ChangeEvent<HTMLInputElement>) => {
    const isValidName = /[^a-zA-ZÀ-ÿ\s]/.test(e.target.value) || e.target.value.length < 3 || e.target.value.length > 15

    setError({
      ...error,
      [e.target.name]: isValidName ? 'No se aceptan caracteres especiales y debe tener entre 3 y 15 caracteres' : null
    })
  }
  ...
  const handlePasswordError = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError({
      ...error,
      password: !isValidPassword ? 'la contraseñase debe tener al menos 8 caracteres' : null
    })
  }
