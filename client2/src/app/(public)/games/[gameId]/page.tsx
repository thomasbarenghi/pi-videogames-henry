...
export default function GameDetails({ params }: { params: { gameId: string } }) {
    ...
    useEffect(() => { dispatch(getGameById(params.gameId)); }, [dispatch, params.gameId]);

    if (isLoading) { return (<Loader theme={"dark"} />) }
    if (isError) { return (<Error theme={"dark"} />) }
                ...
                <div id={styles["hero_col1"]}>
                    ...
                    <div id={styles["col1_card2"]} className={styles["blured-red-box"]}>
                        <p className="body-regular">
                            ...
                            {Array.isArray(currentGame?.platforms) && currentGame?.platforms.map((genre:any) => (
                        ...
                        <p className="body-regular">
                            ...
                            {Array.isArray(currentGame?.genres) && currentGame?.genres.map((genre:any) => (
                                <span key={genre.id}>{genre.name} </span>
                            ))}
                        </p>
                        <p className="body-regular">
