...
const getVideogames = async (req, res) => {
    ...
    if (cachedGames.length > 0) { return res.status(200).json({ "results": cachedGames, "count": cachedGames.length }); }
    ...
    try {
        ...
        for (let currentPage = 1; games.length < 100; currentPage++) {
            let apiGames = [];
            const { data: resGame } = await axios.get(`${API_URL}?key=${API_KEY}&page=${currentPage}&page_size=50`);
            apiGames = await resGame.results.map(res => {
                return {
                    id: res.id,
                    name: res.name,
                    source: "public",
                    genres: res.genres.map(genre => { return { id: genre.id, name: genre.name } }),
                    platforms: res.platforms.map(platform => { return { id: platform.platform.id, name: platform.platform.name } }),
...
const getVideogameById = async (req, res) => {
    ...
    try {
        if (hasLetter) {
            ...
            if (!game) { return res.status(404).json({ error: "No existe el juego" }); }

            const relatedGenres = game.genres.map(genre => ({ id: genre.id, name: genre.name }));
            const relatedPlatforms = game.platforms.map(platform => ({ id: platform.id, name: platform.name }));
            const filteredGame = {
                id: game.id,
                name: game.name,
            ...
            const filteredResponse = filterObjectProperties(game, ["id", "name", "description", "background_image", "rating", "released", "genres", "platforms"]);
            filteredResponse.platforms = filteredResponse.platforms.map(platform => ({ id: platform.platform.id, name: platform.platform.name }));
            filteredResponse.genres = filteredResponse.genres.map(genre => ({ id: genre.id, name: genre.name }));
            return res.status(200).json(filteredResponse);
        }

    } catch (error) { console.error(error); return res.status(500).json({ error: "Error interno del servidor" }); }
}
...
const addVideogame = async (req, res) => {
    ...
    const { name, description, released, rating, platforms, background_image, token, genres } = req.body;
...
const deleteGame = async (req, res) => {
    ...
    try {
        const game = await Game.findByPk(id);
        if (!game) { throw new Error("No existe el juego"); }
        await game.destroy();
